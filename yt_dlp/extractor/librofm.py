...
class LibroFmIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        if username == 'token':
            ...
            self.to_screen(
                f'The access token is {access_token!r}. '
                'Use it and "token" as the username to avoid login requests.')
    ...
    def _process_part(self, part):
        return {
            'formats': [{
                **traverse_obj(part, {
                    'url': ('url', url_or_none),
                    'filesize': ('size_bytes', int_or_none),
                }),
                'ext': 'temp',
        ...
        info = {
            ...
            **traverse_obj(
                ...
                ('data', 'audiobook', {
                    ...
                    'thumbnail': ('cover_url', {self._proto_relative_url}),
                    'description': ('description', {strip_or_none}),
                    # XXX: Does this make sense (publisher being used as the value for channel)?
                    'channel': ('publisher', {str}),
                    'creators': ('authors', {str}),
                    'timestamp': ('created_at', {parse_iso8601}),
                    'release_timestamp': ('publication_date', {parse_iso8601}),
                    'modified_timestamp': ('updated_at', {parse_iso8601}),
                    'duration': ('duration', {float_or_none}),
                    'categories': ('genres', ..., 'name', {str}),
                    'cast': ('audiobook_info', 'narrators', {str}),
                    # XXX: Does this make sense (series and season number)?
                    'series': ('series', {str_or_none}),
                    'season_number': ('series_num', {int_or_none}),
                }),
            ),
        }

        manifest = self._download_json(
            urljoin(self._BASE_URL, '/api/v9/download-manifest'),
            isbn,
            query={'isbn': isbn, 'client_version': self._CLIENT_VERSION},
            headers={
                'Authorization': f'Bearer {self._ACCESS_TOKEN}',
                'User-Agent': self._USER_AGENT,
            }, note='Downloading download manifest JSON',
            errnote='Failed to download download manifest JSON')
